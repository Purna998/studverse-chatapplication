#!/usr/bin/env python3
"""
Test script for profile picture display and message delivery fixes

This script tests:
1. Profile picture display in search results
2. Message delivery when searching and starting a chat
"""
...
def test_profile_and_message_fixes():
    """Test profile picture display and message delivery fixes"""
    print("🧪 Testing Profile Picture and Message Delivery Fixes")
    print("=" * 70)
    ...
    print("\n2. Authenticating receiver (testuser1)...")
    try:
        receiver_login_data = {
            "username": "testuser1",
            "password": "testpass123"
    ...
    # Step 4: Sender searches for receiver by email
    print("\n4. Sender searching for receiver by email (testuser1@example.com)...")
    try:
        search_response = requests.get(f"{BASE_URL}/users/search/?q=testuser1@example.com", headers=sender_headers)
        ...
        if search_response.status_code == 200:
            ...
            if receiver_found:
                log_test("User Search by Email", "PASS", "Receiver found in search results")
                ...
                log_test("User Search by Email", "FAIL", "Receiver not found in search results")
                return False
        else:
            log_test("User Search by Email", "FAIL", f"Search failed: {search_response.status_code}")
            return False
            
    except Exception as e:
        log_test("User Search by Email", "FAIL", f"Error: {e}")
        return False
    
    # Step 5: Sender searches for receiver by name
    print("\n5. Sender searching for receiver by name (testuser1)...")
    try:
        ...
        if search_response.status_code == 200:
            ...
            if receiver_found:
                log_test("User Search by Name", "PASS", "Receiver found in search results")
                receiver_user = next(user for user in search_results if user.get('email') == 'testuser1@example.com')
                print(f"   Found: {receiver_user.get('full_name', 'N/A')} ({receiver_user.get('email', 'N/A')})")
                ...
                profile_picture = receiver_user.get('profile_picture')
                if profile_picture:
                    log_test("Profile Picture in Name Search", "PASS", f"Profile picture found: {profile_picture}")
                else:
                    log_test("Profile Picture in Name Search", "FAIL", "No profile picture in search results")
                    
            else:
                log_test("User Search by Name", "FAIL", "Receiver not found in search results")
                return False
        else:
            log_test("User Search by Name", "FAIL", f"Search failed: {search_response.status_code}")
            return False
            
    except Exception as e:
        log_test("User Search by Name", "FAIL", f"Error: {e}")
        return False
    
    # Step 6: Sender sends message to receiver
    print("\n6. Sender sending message to receiver...")
    try:
        message_data = {
            "receiver_email": "testuser1@example.com",
            "content": "Hello testuser1! This is a test message from testsearch. I found you through search and started this conversation."
        }
    ...
    # Step 7: Wait a moment for WebSocket notifications to be processed
    print("\n7. Waiting for WebSocket notifications...")
    time.sleep(3)
    
    # Step 8: Check if receiver now has the new conversation
    print("\n8. Checking receiver's updated conversations...")
    try:
        ...
        if updated_conversations_response.status_code == 200:
            ...
            if updated_count > initial_count:
                ...
                if new_conversation:
                    ...
                    if last_message and 'Hello testuser1! This is a test message from testsearch' in last_message.get('content', ''):
    ...
    # Step 9: Verify the message content in the conversation
    print("\n9. Verifying message content in conversation...")
    try:
        ...
        if conversation_messages_response.status_code == 200:
            ...
            if messages:
                ...
                if 'Hello testuser1! This is a test message from testsearch' in message_content:
    ...
    print("\n" + "=" * 70)
    print("✅ Profile picture and message delivery test completed!")
    return True

if __name__ == "__main__":
    test_profile_and_message_fixes()
